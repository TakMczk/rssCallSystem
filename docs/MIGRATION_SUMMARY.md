...
## 2026-04-29 更新: 既定値を gpt-5-nano に戻し、コスト優先運用へ調整

`gpt-5.4-nano` 既定化後、API 利用量の観点から現行の既定値は `gpt-5-nano` に戻しました。モデル別 reasoning 設定とキャッシュ分離は維持しているため、`OPENAI_MODEL` を切り替えても既存のキュレーション改善はそのまま使えます。

## 2026-04 更新: gpt-5.4-nano へ既定値を更新

`gpt-5-nano` と `gpt-5.4-nano` を live benchmark で比較し、12記事の代表サンプルで以下を確認しました。

- 速度: **18.15秒 → 10.81秒**（40%高速化）
- コスト: **$0.001124 → $0.003286**（2.92x）
- fallback率: **0% → 0%**
- total score correlation: **0.868**

この結果により、ユーザーが設定した許容コスト上限 **3.5x** を下回ったため、既定モデルを `gpt-5.4-nano` に更新しました。現在は model-aware な reasoning 設定と、モデル別キャッシュ分離を導入済みです。

以下は主に **gpt-4o-mini → gpt-5-nano** 移行の履歴サマリーです。
